# Comprehensive installer for cognis-digital/metascrub (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# metascrub is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "metascrub"
$Url  = "git+https://github.com/cognis-digital/metascrub.git"
$Git  = "https://github.com/cognis-digital/metascrub.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: metascrub"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: metascrub"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: metascrub"; exit 0 }
}
